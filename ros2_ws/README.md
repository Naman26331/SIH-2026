...
> `shared/fleetx_core/`, two directories up, and it is the *same code* the laptop
...
The map lives in **one** place: `shared/fleetx_core/grid.py`. After editing it:

```bash
cd <repo>/ros2_ws/src/fleet_bringup
...
python3 tools/check_purity.py              # proves the brain has no ROS/web code
python3 grid_sim/run.py                    # the browser simulator
python3 -u tools/final_table.py            # the benchmark
```
